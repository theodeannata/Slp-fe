...
interface AppState {
  ...
  // Auth State
  user: User | null;
  role: "admin" | "master" | "pending";
  session: any;
  setAuth: (user: User | null, role: "admin" | "master" | "pending", session?: any) => void;
  logout: () => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  isMockMode: getLocalStorage("slp_mock_mode", true),
  setMockMode: (mock) => {
    set({ isMockMode: mock });
  ...
  role: getLocalStorage("slp_role", "master") as "admin" | "master" | "pending",
  session: null,
  ...
  logout: () => {
    set({ user: null, role: "admin", session: null });
    if (typeof window !== "undefined") {
